...
class ImageProcessor:
    ...
    def generate_segmentation_view(
        ...
        # 5. Overlay corridor boundary and horizon
        ...
        # 7. Render Professional HUD & Legend Overlay
        font = cv2.FONT_HERSHEY_SIMPLEX
        ...
        title_str = "CLASSICAL TRAVERSABILITY & EDGE SEGMENTATION"
        cv2.putText(canvas, title_str, (8, 18), font, 0.42, (0, 225, 255), 1, cv2.LINE_AA)

        fps_val = meta.get('input_fps', 0.0)
        perf_str = f"FPS: {fps_val:.1f} | Latency: {elapsed_ms:.1f} ms"
        cv2.putText(canvas, perf_str, (w - 220, 18), font, 0.36, (100, 255, 100), 1, cv2.LINE_AA)
        ...
        status_str = f"Corridor Traversability: {traversability_pct:.1f}% | Confidence: {confidence_score:.2f} | Method: Classical OpenCV"
